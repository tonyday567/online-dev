...
addLineX y ls cs = cs <> [zeroLine]
  where
    zeroLine = Chart (LineA ls) (SpotPoint <$> [Point lx 0, Point ux 0])
...
digitChart ::
  ...
  [Double] ->
  (HudOptions, [Chart Double])
digitChart title names utcs xs =
  (hudOptions, [c])
  where
    hudOptions =
      defaultHudOptions
...
tableChart tss = zipWith (\ts x -> Chart (TextA defaultTextStyle ts) (sp x <$> [0..])) tss [0..]
...
scannerChart n rs rscan xs =
  ...
  where
    tsrs n rs rscan xs = taker n . (\r -> rscan r xs) <$> rs
